...
def metrics_dataframe(path='/tmp/results', output_path='/tmp'):
    #Create, if not exist, a output_path/"ml" directory in output_path
    Path(output_path, "pretrain").mkdir(parents=True, exist_ok=True)

    # file_list = list(Path(path).glob('*/test_pred/metrics.txt'))
    # For traditional ML classifiers
    file_list = list(Path(path).glob('*/test_pred_nn_classifier/metrics.txt'))
    ...
    data.to_pickle(Path(output_path, "pretrain", "result_full.pkl"))

    data_desc = data.groupby(["model", "fusion", "reducer"]).describe()
    data_desc.to_csv(Path(output_path, "pretrain", "result_desc.csv"))

    simplified_data = data_desc.drop(['max', 'min', '25%', '50%', '75%', 'count'], axis=1, level=1)
    data_desc.to_csv(Path(output_path, "pretrain", "result_desc_small.csv"))
    simplified_data.to_string(Path(output_path, "pretrain", "result_desc_small.txt"))
