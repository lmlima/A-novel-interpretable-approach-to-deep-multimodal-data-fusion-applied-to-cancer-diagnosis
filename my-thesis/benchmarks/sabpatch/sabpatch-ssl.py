...
from raug.loader import get_data_loader, get_data_loader_ssl
...
from sslearning import SSLModel
...
def main(_folder, _csv_path_train, _imgs_folder_train, _lr_init, _sched_factor, _sched_min_lr, _sched_patience,
    ...
    train_ssl_data_loader, train_ssl_data_set = get_data_loader_ssl(train_imgs_path, train_labels, train_meta_data,
                                                            transform=ImgTrainTransform(*transform_param),
                                                            # batch_transform=batch_transform,
                                                            batch_size=_batch_size, shuf=True, num_workers=8,
                                                            pin_memory=True,
                                                            drop_last=True, returnDataset=True
                                                            )
    print("-- SSL training partition loaded with {} images".format(len(train_ssl_data_loader) * _batch_size))
    sslmodel = SSLModel(_model_name, "simclr", train_ssl_data_set, head_size=128)
    sslmodel.train()

    print("-" * 50)
    ####################################################################################################################
    train_data_loader, train_data_set = get_data_loader(train_imgs_path, train_labels, train_meta_data,
                                                        transform=ImgTrainTransform(*transform_param),
                                                        # batch_transform=batch_transform,
                                                        batch_size=_batch_size, shuf=True, num_workers=8,
                                                        pin_memory=True,
    ...
    model = set_model(_model_name, len(_labels_name), neurons_reducer_block=_neurons_reducer_block,
                      ...
                      model_backbone=sslmodel.get_model()
                      )
    ####################################################################################################################
